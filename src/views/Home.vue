          ...
          <form class="createAccDivChaild">
            ...
            <h2>Kreiraj novi račun <span class="exit" @click="showCreateAccDiv(false)"> x </span> </h2>
                ...
                <select v-model="createSelected"  class="inputWrite"  >
                  <option v-for="typ in typeAccount" :key="typ.acc_type_id" class="option"> {{typ.acc_type_name}}</option>
                </select><br>
                <label>Suma</label><br>
                <input type="text"  class="inputWrite" v-model="createSum"><br>
                <label>Naziv računa</label><br>
                <input type="text" class="inputWrite" v-model="createName"><br>
                <input type="button" class="inputWrite" value="Kreiraj Račun" @click="checkFormCreateAcc"><br>
  ...
  <h1 v-if="isLoggedIn"> Naziv računa:<span class="orange">{{defAcc.acc_name}} </span>  Stanje: <span class="orange">{{defAcc.acc_amount}} - {{defAcc.acc_type_name}}</span> </h1><br>
    ...
    <div class="main">
      ...
      <div  v-if="selectTransaction" class="tranType black">
          <h2><span class="orange">Rashod</span> - Potrošnja</h2>
          ...
          <input v-model="buySum" type="number" class="inputWrite"><br>
          <label>Opis</label><br>
          <input v-model="buyDesc" type="text" class="inputWrite"><br>
          <input type="button" value="Unesi" class="inputWrite" @click="checkFormBuy"><br>
      <p v-for="err in buyErrors" :key="err">{{err}}</p>
      </div><!-- end tranType-->

      <!-- add mony to acount -->
      <div v-else class="tranType">
         <h2><span class="orange">Prihod</span> - Uplate</h2>
         <label>Iznos</label><br>
         <input v-model="addSum" type="number" class="inputWrite"><br>
         <label>Opis</label><br>
         <input v-model="addDesc" type="text" class="inputWrite" ><br>
         <input type="button" value="Unesi" class="inputWrite" @click="checkFormAddToAccount"><br>
    ...
    <div class="reverse" >
      <div class="showTransaction" v-for="acc in allTransaction" :key="acc.tra_id">
        ...
        <div>Naziv računa:<span class="orange" >{{acc.acc_name }} </span></div>
        <div>Tip tranakcije:<span class="orange"> {{acc.tra_type_name}}</span> </div>
...
export default {
  ...
  methods:{
    ...
    getTypeAccount(){
      axios.get('http://053n122.mars-e1.mars-hosting.com/api/get/getTypesOfAccounts')
      .then((response)=>{
    ...
    getCategory(){
      axios.get('http://053n122.mars-e1.mars-hosting.com/api/get/getCategory')
      .then((response)=>{
      ...
      if(localStorage.getItem('sid')){
        ...
        .then((response)=>{
          ...
          if(this.accounts !== undefined ){
            ...
            console.log(this.defAcc.acc_name+ ' -defAcc');
    ...
    getTransactions(acc_name){
      ...
      if(acc_name !=='' &&  localStorage.getItem('sid')){
        ...
        axios.post('http://053n122.mars-e1.mars-hosting.com/api/wallet/transactionViewsAccount',{sid:localStorage.getItem('sid'),account:acc_name })
        .then((response)=>{
    ...
    createNewAccount(){
      let sid=localStorage.getItem('sid')
      if(sid){
      axios.post('http://053n122.mars-e1.mars-hosting.com/api/wallet/createAccount',
      {sid:sid, acc_type:this.createSelected, acc_amounth:this.createSum, acc_name : this.createName})
      .then((response)=>{
        this.getAccounts()
    ...
    checkFormBuy(){
      ...
      let sid=localStorage.getItem('sid')
      if(sid){
        axios.post('http://053n122.mars-e1.mars-hosting.com/api/wallet/transactionBuy',
        {sid:sid, acc_id: this.defAcc.acc_id, category: this.categorySelected,tra_amount: this.buySum, tra_description:this.buyDesc  })
        .then((response)=>{
              this.getTransactions(this.defAcc.acc_name)
   ...
   createAddMoney(){
    let sid=localStorage.getItem('sid');
     if(sid){
       axios.post('http://053n122.mars-e1.mars-hosting.com/api/wallet/addMoney',
       {sid:sid, acc_id:this.defAcc.acc_id, tra_amount:this.addSum, tra_description:this.addDesc})
       .then((response)=>{
         this.getTransactions(this.defAcc.acc_name)
         this.message=response.data.message
         this.err=response.data.err
...
.pickOut:hover , .pickOut3:hover{
  cursor: pointer;
   text-shadow: 1.5px 1.5px 1.5px #000000; 
}
.pickOut:active , .pickOut3:active{
  cursor: grab;
